...
let bundle = '/*! Timber v2 - Concatenated Scripts */\n\n';
...
// Update Vite manifest to include the scripts bundle
const manifestPath = join(__dirname, '../dist/.vite/manifest.json');
try {
  const manifestContent = readFileSync(manifestPath, 'utf8');
  const manifest = JSON.parse(manifestContent);

  // Add scripts entry to manifest
  manifest['src/js/scripts.js'] = {
    file: `assets/${filename}`,
    isEntry: true
  };

  writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
  console.log(`\n✓ Updated manifest with scripts entry`);
} catch (err) {
  console.warn(`  Warning: Could not update manifest:`, err.message);
}
