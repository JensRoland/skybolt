...
const BASE_URL = 'http://localhost:8085'; // Different port to avoid conflicts
...
test.describe('Skybolt Ruby Rails Example', () => {
  ...
  test('Cache contains expected assets', async ({ page }) => {
    ...
    expect(paths.some((p: string) => p.includes('application'))).toBe(true);
  });
});
