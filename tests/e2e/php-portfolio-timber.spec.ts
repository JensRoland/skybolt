...
const BASE_URL = 'http://localhost:8081'; // Different port to avoid conflict with php-vanilla

// Expected assets based on render-map.json
...
test.describe('Skybolt PHP Portfolio Timber Example', () => {
  ...
  test('1. Cold cache (first visit) - assets are inlined', async ({ page }) => {
    ...
    // Check no errors (excluding jQuery deprecation warnings which are expected)
    const realErrors = errors.filter((e) => !e.includes('jQuery'));
    expect(realErrors, 'Console error detected').toHaveLength(0);
  ...
  test('Page renders correctly without critical errors', async ({ page }) => {
    ...
    // Check page has content (portfolio site has specific elements)
    await expect(page.locator('body')).toBeVisible();

    // Check no critical JavaScript errors (excluding jQuery deprecation warnings)
    const jsErrors = logs.filter(
      (log) => log.type() === 'error' && !log.text().includes('[Skybolt') && !log.text().includes('jQuery')
    );
  ...
  test('Service Worker responds with correct headers', async ({ page }) => {
    ...
    // Fetch the asset directly (use BASE_URL since this test runs on port 8081)
    const response = await page.request.get(BASE_URL + url.pathname);
